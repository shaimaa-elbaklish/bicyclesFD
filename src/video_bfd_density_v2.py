...
RELEVANT_FRAME = 5800 # from 5715 till 5843
...
def draw_spatiotemporal_bins_on_frame(
    ...
    r_bins, theta_bins, alpha=0.3, cmap_name='RdYlGn',
...
gc.collect()
pfd_df['Global_Time'] = pfd_df['Global_Time'].round(decimals=4)
...
Density_Mat, Flow_Mat, Speed_Mat, Num_Observations_Mat, time_bins, polar_x_bins, polar_y_bins = estimate_traffic_states(
    pfd_df, df_final_trajectory, dx, dy, dt, CRB_Config.video_lane_widths[RELEVANT_VIDEO], 
    config=CRB_Config, mode="XY"
)
Num_Observations_Mat = Num_Observations_Mat.astype(int)

r_outer = CRB_Config.circle_outer_radius
r_inner = r_outer - CRB_Config.video_lane_widths[RELEVANT_VIDEO]
py_mask = (polar_y_bins[:-1] >= r_inner) & (polar_y_bins[1:] <= 15)
Density_Mat = Density_Mat[:, :, py_mask]
Flow_Mat = Flow_Mat[:, :, py_mask]
Speed_Mat = Speed_Mat[:, :, py_mask]
Num_Observations_Mat = Num_Observations_Mat[:, :, py_mask]
polar_y_bins = polar_y_bins[(polar_y_bins >= r_inner) & (polar_y_bins <= r_outer)]

Speed_Mat[Num_Observations_Mat <= 0] = 20.0
...
t_idx = np.digitize(frame_time, time_bins, right=True) - 1
frame_bfd_states = {'Density': Density_Mat[t_idx], 'Flow': Flow_Mat[t_idx], 'Speed': Speed_Mat[t_idx],
                    'Num_Observations': Num_Observations_Mat[t_idx]}
...
ts_overlay = draw_spatiotemporal_bins_on_frame(
    overlay, frame_bfd_states, frame_homography, bbox, r_inner, r_outer, 
    r_bins=polar_y_bins, theta_bins=polar_x_bins, alpha=0.25, cmap_name='RdYlGn', 
    vmin=0, vmax=20, value_column='Speed', smooth_sigma=1.5
)
plt.figure(figsize=(5,5))
...
current_density = frame_bfd_states['Density'].flatten()
weights1 = np.minimum(1, frame_bfd_states['Num_Observations'].flatten() / 10)
weights2 = np.minimum(1, current_density / max_density)
weights = weights1 # weights1*weights2/np.sum(weights1*weights2)
...
# #############################################################################
output_video_path = os.path.join(CRB_Config.video_path, RELEVANT_VIDEO.split('.')[0]+"_"+RELEVANT_PART+"_BFD_V2.mp4")
...
for frame_nr in tqdm(range(RELEVANT_FRAME_FROM, RELEVANT_FRAME_TO+1), total=RELEVANT_FRAME_TO-RELEVANT_FRAME_FROM+1, desc="Rendering video"):
    ...
    overlay = draw_spatiotemporal_bins_on_frame(
        overlay_frame, frame_bfd_states, frame_homography, bbox, r_inner, r_outer, 
        r_bins=polar_y_bins, theta_bins=polar_x_bins, alpha=0.25, cmap_name='RdYlGn', 
        vmin=0, vmax=20, value_column='Speed', smooth_sigma=0.5
    )
    ...
    current_density = frame_bfd_states['Density'].flatten()
    weights1 = frame_bfd_states['Num_Observations'].flatten() / MIN_OBS
